; ============================================================================
; ACVPALT.ASM - CGA Palette TSR for the ACV-1030 ISA video card (V6355)
; Written for NASM - Intel 80286 (12 MHz ACV-1030 target)
; By Retro Erik - 2026 using VS Code with GitHub Copilot
; Version 1.0 - ACV-1030 port of PC1PALT v1.2
; ============================================================================
;
; HARDWARE VALIDATION (2026-08-15): Confirmed working on a 12 MHz 80286
; with an ACV-1030 in three scenarios: ordinary CGA games via CGA/TTL
; (fixed IRGB colors), ordinary CGA games via composite (custom palette
; colors visible), and composite-mode games via composite (custom palette
; colors visible).
;
; Loads custom RGB palettes and programs the Yamaha V6355 DAC, then stays
; resident to survive game mode resets and provide live palette adjustment.
;
; Ported from PC1PALT.ASM (Olivetti Prodest PC1, NEC V40/80186). The original
; PC1PALT source is preserved unchanged at PC1-Palette-Loader\PC1PALT.asm as
; the reference implementation; this file carries the ACV-1030-specific
; hardware changes only.
;
; ACV-1030 PORTING NOTES (every changed assumption vs. PC1PALT):
;   Ports:    PC1 uses the short port aliases 0xD8/0xD9/0xDD/0xDE. The
;             ACV-1030 is documented and accessed through the full 0x3D8/
;             0x3D9/0x3DD/0x3DE addresses. All register-bank and mode I/O in
;             this file is done with DX loaded from a word constant and
;             `out dx, al` - never an immediate `out imm8, al` - because an
;             8-bit immediate operand cannot address a port above 0xFFh.
;   Timing:   V6355 register 0x65 is set to 0x01 (200 lines, NTSC, color,
;             CRT) instead of the PC1's 0x09 (PAL), matching the ACV-1030's
;             validated NTSC-timed composite path (see 1030-col.asm).
;   Bus mode: V6355 register 0x67 is set to 0x18 (8-bit bus, no 64KB paging)
;             once at TSR install so the register-bank/palette interface
;             behaves predictably; PC1PALT never touches 0x67 because the
;             PC1 BIOS/hardware already leaves it in a compatible state.
;   Unlock:   0x80 is written to 0x3DDh to close the register/palette bank
;             before any other register-bank access, matching the ACV-1030's
;             documented safe-unlock sequence. This TSR never sends 0x4Ah to
;             0x3D8h (the 160x200x16 hidden-mode unlock) - it stays a CGA
;             mode 4/5 (320x200x4) palette TSR exactly like PC1PALT; there is
;             no VRAM framebuffer or drawing code in either version.
;   Palette:  The 40h-open / 32-byte-write-with-delays / 80h-close sequence
;             from PC1PALT is preserved unchanged and drives the ACV-1030's
;             composite (NTSC) DAC output. Ordinary CGA games therefore show
;             fixed IRGB on CGA/TTL, but custom palette colors on composite;
;             composite-mode games also show custom palette colors on
;             composite. The CGA/TTL connector itself remains fixed IRGB.
;   CPU:      Targets `[CPU 286]` explicitly (12 MHz 80286, not the NEC V40).
;             No 80186-only opcodes were used by PC1PALT that the 80286
;             doesn't already support (pusha/popa/push imm/shl reg,imm8 are
;             valid on both), so no instruction substitutions were required.
;             `CPU 286` also makes NASM reject any 386-only near conditional
;             jump (0F 8x/0F 9x) at assemble time.
;
; Based on PC1PAL v0.9 (one-shot palette loader) and PalSwapT v3.3 (CGA
; Palette TSR for VGA/EGA), adapted for the ACV-1030's Yamaha V6355 Display
; Adapter Controller.
;
; VERSION HISTORY:
...
;
; HOW IT WORKS:
;   1. Run ACVPALT with your palette choice before starting a game.
;   2. ACVPALT loads the palette, hooks INT 09h + INT 10h, stays resident.
;   3. When a game calls INT 10h AH=00h AL=04h/05h (set CGA mode):
;      - The original handler sets the mode.
;      - Our hook re-programs the V6355 DAC with the custom palette via the
;        full 0x3DD/0x3DE ports (composite/NTSC output only; CGA/TTL IRGB
;        stays fixed).
;   4. Hold Ctrl+Alt + hotkeys for live adjustment while the game runs.
;   5. Re-run ACVPALT with new arguments to update the resident palette.
;   6. Run "ACVPALT /U" to uninstall.
;
; V6355 PALETTE FORMAT:
...
; --- ACV-1030 Yamaha V6355 I/O Ports (full 0x3D* addresses, DX-based OUT) ---
PORT_MODE           equ 0x3D8       ; Mode Control Register (CGA compatible)
PORT_COLOR          equ 0x3D9       ; Color Select Register (border/palette idx)
PORT_REG_ADDR       equ 0x3DD       ; Register Bank Address Port
PORT_REG_DATA       equ 0x3DE       ; Register Bank Data Port
PAL_WRITE_EN        equ 0x40        ; Enable palette write
PAL_WRITE_DIS       equ 0x80        ; Disable palette write / close register bank

; --- V6355 configuration registers (written once at install, see acv1030_init) ---
REG_BUS_MODE        equ 0x67        ; Bus/paging configuration register
REG_BUS_MODE_VALUE  equ 0x18        ; 8-bit bus, no 64KB page mode
REG_MONITOR_CTRL    equ 0x65        ; Monitor timing/type register
REG_MONITOR_VALUE   equ 0x01        ; 200 lines, NTSC, color, CRT

; --- Palette sizes ---
CONFIG_SIZE         equ 12          ; 4 colors x 3 bytes (single palette RGB)
PALETTE_ENTRIES     equ 4           ; User-defined colors
FULL_PALETTE        equ 16          ; V6355 palette entries
...
; --- Default CGA palette in V6355 format (16 x 2 bytes = 32 bytes) ---
cga_full_palette:
...
; --- V6355 work buffers (resident) ---
user_colors:        times 8 db 0    ; 4 user colors x 2 bytes V6355 format
palette_buffer:     times 32 db 0   ; 16 colors x 2 bytes V6355 format
...
; generation + V6355 DAC write). EOI is sent to the PIC in .swallow.
; ============================================================================
tsr_int09:
    push ax
    push bp
...
hotkey_load_fallback:
    ...
    ; Text mode: full V6355 restore, go dormant
...
;                       convert to V6355 format and write to hardware.
;
...
;            -> convert to V6355 and write to DAC
; ============================================================================
recompute_and_apply:
    pusha
    push ds
...
.rc_apply:
    ; Step 5: Convert and write to V6355
    call res_apply_palette
...
; ============================================================================
; res_apply_palette — Convert palette_rgb to V6355 format and write to DAC.
...
res_apply_palette:
    ...
    ; Step 1: Convert 4 RGB colors (6-bit) to V6355 format in user_colors
    mov si, palette_rgb
    mov di, user_colors
    mov cx, 4
.convert:
...
.build:
    ...
    xor ah, ah
    shl al, 1                  ; x 2 bytes per V6355 color
    mov bx, ax
    mov ax, [user_colors + bx] ; 2 bytes of V6355 data
    stosw
    loop .build

    ; Step 3: Write to V6355 DAC
...
; ============================================================================
; res_write_palette_to_dac — Write palette_buffer (32 bytes) to V6355
...
; ============================================================================
; res_restore_default — Write standard CGA palette to V6355 DAC
; ============================================================================
res_restore_default:
    pusha
    push ds
...
; ============================================================================
main:
    ; ACV-1030 port: DOS guarantees DS=CS=ES=SS at COM entry, but set both
    ; explicitly and clear the direction flag before any lodsb/stosb/movsw use.
    push cs
    pop ds
    cld

    ; ACV-1030 port: configure the V6355 bus/timing registers once per run.
    ; This does not change video mode - it only prepares the register bank
    ; so later palette writes (and any game's own mode 4/5 set) behave
    ; correctly on the ACV-1030's 8-bit bus.
    call acv1030_init
...
; ============================================================================
; acv1030_init — ACV-1030 port: one-time V6355 register setup.
;   Register 0x67 = 0x18 (8-bit bus, no 64KB page mode)
;   Register 0x65 = 0x01 (200 lines, NTSC, color, CRT)
; Every register-bank access goes through DX (never an 8-bit immediate OUT,
; since 0x3DDh/0x3DEh are above 0xFFh) and 0x80h is written to 0x3DDh first
; to leave the bank in its documented closed/safe state before selecting a
; register. This does not enable the 160x200x16 hidden mode (no 0x4Ah is
; sent to 0x3D8h) - ACVPALT stays a CGA mode 4/5 palette TSR, same as PC1PALT.
; ============================================================================
acv1030_init:
    push ax
    push dx
    ...
    mov dx, PORT_REG_ADDR
    mov al, PAL_WRITE_DIS       ; 0x80: close/select bank safely first
    out dx, al
    jmp short $+2

    mov dx, PORT_REG_ADDR
    mov al, REG_BUS_MODE        ; select register 0x67
    out dx, al
    jmp short $+2
    mov dx, PORT_REG_DATA
    mov al, REG_BUS_MODE_VALUE  ; 0x18: 8-bit bus, no paging
    out dx, al
    jmp short $+2

    mov dx, PORT_REG_ADDR
    mov al, REG_MONITOR_CTRL    ; select register 0x65
    out dx, al
    jmp short $+2
    mov dx, PORT_REG_DATA
    mov al, REG_MONITOR_VALUE   ; 0x01: 200 lines, NTSC, color, CRT
    out dx, al
    jmp short $+2

    mov dx, PORT_REG_ADDR
    mov al, PAL_WRITE_DIS       ; 0x80: leave the register bank closed
    out dx, al
...
check_uninstall:
    push si
    ; ACV-1030 port: zero-length DOS command tail (byte [0x80] = 0) must not
    ; read DS:0081h at all - some DOS versions/loaders leave it uninitialized.
    cmp byte [0x80], 0
    je .none
    mov si, 0x81
.skip:
...
check_switches:
    push si
    ; ACV-1030 port: skip parsing entirely on a zero-length command tail.
    cmp byte [0x80], 0
    je .no_switch
    mov si, 0x81
...
msg_banner:
    db 'ACVPALT v1.0 - CGA Palette TSR for ACV-1030', 13, 10
    db 'By Retro Erik - 2026 - Yamaha V6355 DAC Programmer', 13, 10
    db 'Type: ACVPALT /? for help', 13, 10, '$'

msg_help:
    db 13, 10
    db 'Usage: ACVPALT [file.txt] [/0..9] [/c:c1,c2,c3] [/b:color]', 13, 10
    db '               [/P] [/V:+|-] [/D:+|-] [/R] [/U] [/?]', 13, 10
    db 13, 10
...
msg_installed:
    ...
    db 'Run your CGA game now. Use ACVPALT /U to uninstall.', 13, 10, '$'
msg_already_loaded:
    db 'ACVPalT already resident - palette updated.', 13, 10, '$'
msg_unloaded:
    db 'ACVPalT uninstalled. INT 09h + INT 10h restored.', 13, 10, '$'
msg_unload_error:
    db 'Error: ACVPalT not found in interrupt chain.', 13, 10, '$'
...
msg_success:     db 'Palette written to V6355 DAC.', 13, 10, '$'

default_filename: db 'ACVPALT.TXT', 0
